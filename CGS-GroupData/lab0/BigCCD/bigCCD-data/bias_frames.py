...
#Plot and fit bias
def plotbias(path,temp):
    if temp == 'Neg5':
        filename = 'biases-Neg5-Vis.00000000.BIAS.FIT'
    if temp == 'Neg10':
        filename = 'bias-Neg10-Vis.00000000.BIAS.FIT'
    print jasminepath + '/' + filename
    hdulist = fits.open(jasminepath + '/' + filename)
...
def mastbi(path,temp): #path,temperature word in file name
    all_bias = []
    ...
    sig_single = plotbias(path,temp)
    print "Mean all = ", mean_all
    print "Sig all = ", sig_all
    N = len(all_bias) #number of images used for master bias
    calc_sig_single = sig_single * (1/np.sqrt(N))
    print "Calculated standard deviation of master_bias", calc_sig_single
    
    return


#plotbias(jasminepath,"Neg5")
#mastbi(jasminepath,"Neg5")
plotbias(jasminepath,"Neg10")
mastbi(jasminepath,"Neg10")
